...
exports.login = catchAsync(async(req,res,next)=>{
    ...
    //2)check user exist or not and password is correct or not
...
exports.protect = catchAsync(async (req,res,next)=>{
    ...
    //Getting token and check iof its there
    let token; 
    ...
    //4) checking password is changed or not
...
exports.restrictTo = (...roles)=>{
    return (req,res,next)=>{
         ...
         if(!roles.includes(req.user.role)){
            return next(new AppError('Permission restricted',403));
         }
         next();
    };
};
